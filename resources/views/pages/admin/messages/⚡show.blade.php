...
<main class="flex-1">
    ...
    {{-- Header --}}
    <div
        class="flex flex-col md:flex-row justify-between items-start md:items-center px-5 md:px-6 py-4 gap-4 mb-8 max-w-7xl mx-auto">
        <div>
            ...
            <div class="flex items-center gap-3">
                ...
                />
                <span
                    class="text-sm text-grayscale-text-subtitle">Reçu le {{ $message->created_at->format('d/m/Y à H:i') }}</span>
            </div>
        </div>
            ...
            <div class="flex items-center gap-4 mb-6 pb-6 border-b border-neutral-100">
                ...
                <div>
                    ...
                    <a href="mailto:{{ $message->email }}"
                       class="text-primary-text-link-light hover:underline">{{ $message->email }}</a>
                    @if($message->phone)
                        <span class="text-neutral-400 mx-2">•</span>
                        <a href="tel:{{ $message->phone }}"
                           class="text-grayscale-text-subtitle hover:text-grayscale-text-body">{{ $message->phone }}</a>
                    @endif
                </div>
            </div>
