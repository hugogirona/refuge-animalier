...
: false }" @click.away="open = false">
    ...
    <div
        ...
        class="absolute right-0 mt-2 w-56 bg-white rounded-lg shadow-lg border border-neutral-200"
        style="display: none;"
    >
        <a href="{{ route('settings.index') }}" class="flex items-center gap-3 px-4 py-2 text-sm text-grayscale-text-subtitle hover:bg-neutral-50 transition-colors">
        ...
        <form method="POST" action="{{ route('login') }}">
            @csrf
            <button type="submit" class="w-full flex items-center gap-3 px-4 py-2 text-sm text-error-text-link-light hover:bg-error-surface-default-subtle transition-colors">
                <svg class="w-5 h-5 fill-none" stroke="currentColor" viewBox="0 0 24 24">
