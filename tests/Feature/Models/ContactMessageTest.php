...
describe('ContactMessage Model', function () {
    ...
    it('can filter new messages', function () {
        ...
        expect(ContactMessage::new()->count())->toBe(3)
            ->and(ContactMessage::read()->count())->toBe(2);
    });

    it('can filter replied messages', function () {
        ContactMessage::factory()->replied()->create();
        ContactMessage::factory()->create(['status' => ContactMessageStatus::NEW]);

        expect(ContactMessage::replied()->count())->toBe(1);
    });
